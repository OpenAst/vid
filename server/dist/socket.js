...
exports.likeSystem = exports.setupCommentSocket = void 0;
const axios_1 = __importDefault(require("axios"));
const dotenv_1 = __importDefault(require("dotenv"));
dotenv_1.default.config();
const setupCommentSocket = (io, redisClient) => {
    const commentIo = io.of('/comments');
    // Middleware to authenticate socket via JWT cookie
    commentIo.use((socket, next) => {
    ...
    commentIo.on('connection', (socket) => {
        ...
        socket.on('send-comment', async ({ text, roomId, user }) => {
            try {
                const res = await axios_1.default.post(`${process.env.DJANGO_API_URL}/api/comments/create/${roomId}/`, {
                    video: roomId,
                    user: user.id,
                    content: text,
                }, {
        ...
        socket.on('vote-comment', async ({ commentId, roomId, userId }) => {
            try {
                ...
                await axios_1.default.post(`${process.env.DJANGO_API_URL}/api/comments/vote/`, {
                    value: 1,
                    commentId,
                }, {
                    headers: {
        ...
        socket.on('send-reply', async ({ parentId, text, roomId, user }) => {
            try {
                ...
                const res = await axios_1.default.post(`${process.env.DJANGO_API_URL}/api/comments/reply/${parentId}/`, {
                    user: user.id,
                    content: text,
                }, {
                    headers: {
...
exports.setupCommentSocket = setupCommentSocket;
// Video Like System
const likeSystem = (io, redisClient) => {
    const videoLike = io.of('/video-likes');
    // Reuse existing auth middleware
    videoLike.use((socket, next) => {
        const cookieHeader = socket.handshake.headers.cookie || '';
        const cookies = Object.fromEntries(cookieHeader
            .split(';')
            .map(c => c.trim().split('=').map(decodeURIComponent)));
    ...
    videoLike.on('connection', (socket) => {
        ...
        socket.on('like-video', async ({ videoId, userId }) => {
            ...
            try {
                // Persist to Django
                const res = await axios_1.default.post(`${process.env.DJANGO_API_URL}/api/videos/vote/`, {
                    video: videoId,
                    value: 1
                }, {
                    headers: {
